...
plt.ylabel('Wind speed difference (SAR-SMAP(RSS)) (m/s)',fontsize=16)
plt.grid()
plt.tick_params(axis = 'both', labelsize = 16)
plt.errorbar((_[1:] + _[:-1])/2, mean, yerr=std, fmt='r-',capthick = 2)
savefig('/net/merzhin1/vol/safo/morgane/data/figures/STATS_SAR_SMAP(REMSS)_GLOB/Wind_speed_SAR-SMAP(REMSS)=f(SAR_mean_angle_incidence)_dt<10000_pyre.png')


fig_2 = figure(figsize = (10,7))
fig_2.suptitle('Wind speed (dt<2h45min)', fontsize = 18)
plt.plot([0,100],[0,100],c='k')
im = scatter(vent_smap_1,vent_sar_1,marker='.',alpha = 0.2,s=25)
plt.grid()
...
plt.text(1,44,'std = '+str(round(std_diff,2))+' m/s',fontsize=16)
plt.xlabel('SMAP(RSS) Wind speed (m/s)',fontsize=16)
plt.ylabel('SAR Wind speed (m/s)',fontsize=16)
plt.tick_params(axis = 'both', labelsize = 16)
xlim(0,70)
ylim(0,70)
savefig('/net/merzhin1/vol/safo/morgane/data/figures/STATS_SAR_SMAP(REMSS)_GLOB/Wind_speed(SAR&SMAP(REMSS))_dt<10000_pyre.png')

fig_3 = figure(figsize = (10,7))
fig_3.suptitle('Difference between Sentinel-1 and SMAP(RSS) wind speed (dt<2h45min)', fontsize = 18)
im = plt.hist(diff_smap.compressed(),bins=50)
plt.grid()
plt.xlabel('Wind speed difference',fontsize=16)
plt.xlim(-20,20)
plt.tick_params(axis = 'both', labelsize = 16)
savefig('/net/merzhin1/vol/safo/morgane/data/figures/STATS_SAR_SMAP(REMSS)_GLOB/Hist(SAR-SMAP(REMSS))_dt<10000_pyre.png')

fig_4 = figure(figsize = (10,7))
fig_4.suptitle('Wind speed difference (SAR-SMAP(RSS)) vs delta T (dt<2h45min)', fontsize = 18)
im = scatter(dt_1,diff_smap,marker='.' )
plt.xlabel('Difference between time acquisition ( s )',fontsize=16)
plt.ylabel('Wind speed difference (SAR-SMAP(RSS)) (m/s)',fontsize=16)
plt.grid()
plt.tick_params(axis = 'both', labelsize = 16)
savefig('/net/merzhin1/vol/safo/morgane/data/figures/STATS_SAR_SMAP(REMSS)_GLOB/Diff(SAR-SMAP_R)_vs_dt<10000_pyre.png')
file_stat.close()
plt.close('all')
print(R_1)
