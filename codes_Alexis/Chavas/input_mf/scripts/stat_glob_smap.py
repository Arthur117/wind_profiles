...
#	j='/net/merzhin1/vol/safo/morgane/data/IRMA/netcdf_intermediaire/IRMA_20170907_SAR_s1a_SMAP.nc'
	...
	vent_smap = np.squeeze(file_stat.variables['WindSpeed_smap'][:]) # SMAP
...
#	if delta < 20000:
	...
	R = np.ma.corrcoef(vent_smap,vent_sar)
	diff_moy = diff.mean()
	diff_med = np.ma.median(diff)
	nb_sum = vent_smap.shape-vent_smap.mask.sum()
	...
	try:
		...
		ax3.set_title('Number of points by pixel SMAP(REMSS)')
		...
		savefig('/net/merzhin1/vol/safo/morgane/data/figures/STATS_SAR_SMAP(REMSS)_RESUME/STATS_SAR_SMAP(REMSS)/'+name_cycl+'_'+date1+'_SAR_SMAP(REMSS)_compare(@SMAP(REMSS)_res).png')
		...
		xlim(0,70)
		ylim(0,70)
		xlabel('Vent SMAP(REMSS) (m/s)',fontsize=12)
		ylabel('Vent Sentinel-1 (m/s)',fontsize=12)
		savefig('/net/merzhin1/vol/safo/morgane/data/figures/STATS_SAR_SMAP(REMSS)_RESUME/STATS_SAR_SMAP(REMSS)/'+name_cycl+'_'+date1+'_SAR_vs_SMAP(REMSS).png')


		fig_2 = figure(figsize = (10,7))
		fig_2.suptitle(name_cycl+' '+date+' '+'Difference between Sentinel-1 and SMAP(REMSS) wind speed', fontsize = 18)
		im = plt.hist(diff.compressed(),bins=50)
		xlim(-lim-1,lim+1)
		xlabel('Wind speed difference (SAR-SMAP(REMSS)) (m/s)',fontsize=12)
		ylabel('Number of points',fontsize=12)
		plt.grid()
		savefig('/net/merzhin1/vol/safo/morgane/data/figures/STATS_SAR_SMAP(REMSS)_RESUME/STATS_SAR_SMAP(REMSS)/'+name_cycl+'_'+date1+'_SAR_SMAP(REMSS)_WindSpeed_difference(hist).png')
		...
		plt.ylabel('Difference between SAR & SMAP(REMSS) (m/s)', fontsize = 12)
		savefig('/net/merzhin1/vol/safo/morgane/data/figures/STATS_SAR_SMAP(REMSS)_RESUME/STATS_SAR_SMAP(REMSS)/'+name_cycl+'_'+date1+'_SAR-SMAP(REMSS)_vs_angle.png')
