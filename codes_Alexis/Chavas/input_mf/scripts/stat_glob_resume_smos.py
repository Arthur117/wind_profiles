...
for i,j in enumerate(path):
	print(i,j)
	#j='/net/merzhin1/vol/safo/morgane/data/netcdf_intermediaire/IRMA_20170907T103700_SAR_s1a_SMOS.nc'
	...
	vent_diffu = np.squeeze(file_stat.variables['WindSpeed_smos'][:]) # SMOS
	...
	try:
		...
		ax3.set_title('Difference (SAR-SMOS) (m/s)')
		plt.colorbar(im, cax = ax9, orientation='vertical', label='SAR-SMOS (m/s)')
		...
		im = m.pcolormesh(x1,y1,nb_points,cmap='jet', ax = ax4,vmin = 400,vmax = 1000)
		m.drawcoastlines(ax = ax4)
		m.fillcontinents(color='grey',ax = ax4)
		ax4.set_xlabel('Longitude', labelpad = 20, fontsize=12)
		ax4.set_title('Number of points by pixel SMOS')
		...
		ax5.set_xlim(0,70)
		ax5.set_ylim(0,70)
		ax5.set_xlabel('SMOS Wind speed (m/s)', fontsize=12)
		ax5.set_ylabel('SAR Wind speed (m/s)', fontsize=12)
		ax5.set_title('SMOS Wind speed vs SAR wind speed ')

		ax6.hist(diff.compressed(),bins=50)
		ax6.set_xlim(-lim-1,lim+1)
		ax6.grid()
		ax6.set_xlabel('Wind speed difference (SAR-SMOS) (m/s)', fontsize=12)	
		...
		ax7.set_ylabel('Wind speed difference (SAR-SMOS) (m/s)', fontsize=12)
		ax7.set_title('Wind speed difference vs incidence angle')

		savefig('/net/merzhin1/vol/safo/morgane/data/figures/STATS_SAR_SMOS_RESUME/'+name_cycl+'_'+date1+'_SAR_SMOS_stat_pyre.png')
